...
public final class DeathNoteScreen extends Screen {
    ...
    protected void init() {
        ...
        int top = Math.max(45, this.height / 2 - 70);

        this.targetBox = new EditBox(this.font, centerX - 100, top, 200, 20, Component.translatable("screen.deathnote_realistic.target"));
        this.targetBox.setMaxLength(DeathNoteRules.MAX_PLAYER_NAME_LENGTH);
        this.targetBox.setHint(Component.translatable("screen.deathnote_realistic.target_hint"));
    ...
    private void submit() {
        ...
        ClientPlayNetworking.send(new DeathNoteWritePayload(target, this.cause.id()));
        onClose();
    }
